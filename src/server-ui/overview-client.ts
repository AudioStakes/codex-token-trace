...
const findRowBySessionId = (sessionId) =>
  getRows().find((row) => row.dataset.sessionId === sessionId) ?? null;

export const updateRowClasses = () => {
  // biome-ignore lint/complexity/noForEach: small DOM list, imperative row updates are clearer here.
  getRows().forEach((row) => {
...
const scheduleRowScroll = () => {
  if (
    !state.leftmostVisibleSessionId ||
    state.leftmostVisibleSessionId === state.lastScrolledSessionId
  ) {
    return;
  }
...
const setSummary = (summary) => {
  ...
  document.getElementById("summary-compactions").textContent = fmtInt(
...
export const renderRankingTable = () => {
  ...
  const cells = sessions
    .map((session) => {
      ...
      return (
        ...
        '<div class="session-path" title="' +
        esc(path) +
        '">' +
        esc(path) +
        "</div>" +
        '<div class="session-id mono" title="' +
        esc(session.sessionId) +
        '">' +
        esc(session.sessionId) +
        "</div>" +
        ...
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(Math.max(0, session.nonCachedInputTokens ?? 0)) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(Math.max(0, session.finalCachedInputTokens ?? 0)) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(Math.max(0, session.visibleOutputTokens ?? 0)) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(Math.max(0, session.finalReasoningOutputTokens ?? 0)) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(Math.max(0, session.adjustmentTokens ?? 0)) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(session.events) +
        "</td>" +
        '<td class="mono numeric">' +
        fmtInt(session.compactions) +
        "</td>" +
        "<td>" +
        esc(dominant) +
        "</td>" +
        ...
        esc(likelyDriver) +
        "</td>" +
        '<td class="command-preview" title="' +
        esc(topCommandPreview) +
        '">' +
        esc(topCommandPreview) +
        "</td>" +
        "</tr>"
  ...
  rankingBody.innerHTML =
    cells || '<tr><td colspan="13" class="detail-empty">No session data.</td></tr>';
  // biome-ignore lint/complexity/noForEach: small DOM list, imperative row updates are clearer here.
  getRows().forEach((row) => {
...
export const renderChart = () => {
  ...
  // biome-ignore lint/complexity/noForEach: canvas drawing stays imperative here.
...
// biome-ignore lint/complexity/noForEach: tooltip binding is a one-time DOM pass.
...
loadOverview().catch((error) => {
  console.error(error);
  rankingBody.innerHTML =
    '<tr><td colspan="13" class="detail-empty">Failed to load overview: ' +
    esc(error.message) +
    "</td></tr>";
});
